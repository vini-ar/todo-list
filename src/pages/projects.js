...
import {
  ...
  renderSidebarAddTaskFormContainer,
  renderTaskPage,
} from "../components/renderElements";
import { taskManager } from "../components/taskManager";
import {
  handleProjectFormCancelButtonClick,
  handleProjectFormSubmitButtonClick,
} from "../components/projectFormHandleActions";
...
export const manageProjectUI = {
  ...
  getDOM() {
    ...
    this.sidebarFormContainer = document.querySelector(
      "#projectSidebarFormContainer",
    );
    this.contentAddProjectButton = document.querySelector(
      "#contentCreateProjectButton",
    );
    this.sidebarAddProjectButton = document.querySelector(
      "#sidebarCreateProjectButton",
    );
    this.sidebarAddTaskButton = document.querySelector(
  ...
  attachProjectPageEventListener() {
    ...
    this.sidebarAddTaskButton?.addEventListener("click", (e) =>
      renderSidebarAddTaskFormContainer(e.target),
    );
  },
  attachCreateFormEventListener(createProjectForm) {
    this.cancelProjectFormButton = createProjectForm.querySelector(
      ".projectFormCancelButton",
    );
    this.submitProjectFormButton = createProjectForm.querySelector(
  ...
  renderProjectPage(targetProjectId) {
    ...
    const header = elementFactory("h1", Project.name, {
