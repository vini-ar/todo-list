...
import {
  ...
  attachTaskFormActionListeners,
} from "./taskAttachEventListener";
import { renderTask } from "./renderTask";
import { taskFormFactory } from "./taskFormFactory";
import {
  getColors,
  getUserProjecstLength,
  getUserProjectsList,
} from "./projectManager";
...
export function renderPriorityContainer() {
  ...
  const newPriorityContainer = elementFactory("div", "", {
    class: "priorityContainer",
  });
  for (let i = 1; i < 5; i++) {
    const button = elementFactory("button", `Priority ${i}`, {
      id: `priority-p${i}`,
...
export function createDateContainer() {
  const dateContainer = elementFactory("div", "", {
    class: "userDateContainer",
  });
...
export function renderUserTask() {
  ...
  const inboxTasks = userTasksDatabase.filter(
...
function renderProjectColorOptions(form) {
  ...
  colors.forEach((color) => {
    const colorOption = elementFactory("option", color, {
      value: color,
    });
...
function renderParentProjectOptions(formContainer) {
  const projectParentSelect = formContainer.querySelector(
    "#projectParentSelect",
  );
  ...
  if (userProjectsLength !== 0) {
    userProjects.forEach((project) => {
      const projectOption = elementFactory("option", project, {
        value: project,
      });
  ...
  const projectParentDefaultOption = elementFactory("option", "No parent", {
    value: "No parent",
    selected: "selected",
  });
...
export function renderProjectItem(Project) {
  const projectItemDiv = elementFactory("div", Project.name, {
...
export function renderTaskPage(button) {
  ...
  const taskUpdatePageContainer = elementFactory("div", "", {
    class: "task-update-page-container",
  });
  content.append(taskUpdatePageContainer);
  const taskUpdatePage = elementFactory("div", "teste", {
    id: "task-update-page",
    "data-task-id": Task.id,
  });
  taskUpdatePage.innerHTML = `<div class="controlls">
  ...
  const updateTaskQuitButton = taskUpdatePage.querySelector(
    "#updateTaskQuitButton",
  );
  const updatePriorityDiv = taskUpdatePage.querySelector(
  ...
  updateTaskQuitButton.addEventListener("click", (e) => {
    ...
    const taskItemDecription = taskItem.querySelector(
  ...
  updateNameDiv.addEventListener("click", () => {
    ...
    const updateInput = elementFactory("input", "", {
      type: "text",
      id: "update-task-input",
    });
    const updateButton = elementFactory("button", "Save", {
      type: "button",
      id: "update-button-name",
    });
    updateButton.addEventListener("click", (e) => {
      e.stopPropagation();
      const updadateTaskInput =
        updateNameDiv.querySelector("#update-task-input");
      const newTaskName = updadateTaskInput.value;
      taskManager.updateTaskName(taskId, newTaskName);
      updateInput.remove();
      updateButton.remove();
      const updateTaskNameDiv =
        taskUpdatePage.querySelector("#task-name-update");
  ...
  const updateDescriptionDiv = taskUpdatePage.querySelector(
  ...
  updateDescriptionDiv.addEventListener("click", () => {
    ...
    const updateDescriptionButton = elementFactory("button", "Save", {
    ...
    updateDescriptionButton.addEventListener("click", (e) => {
      ...
      if (updateDescriptionDiv) {
        ...
        updateDescriptionDiv.textContent =
          "Description: " + taskManager.getTaskDescription(Task.id);
      }
  ...
  updatePriorityDiv.addEventListener("click", (e) => {
    ...
    let priorityContainer =
      updatePriorityDiv.querySelector(".priorityContainer");
    if (!priorityContainer) {
